...
class BARTTextClassifier(nn.Module):
    def __init__(self, num_labels=2):
        super(BARTTextClassifier, self).__init__()
        self.bart = BartForSequenceClassification.from_pretrained('facebook/bart-base', num_labels=num_labels)
...
def load_pretrained_bart_weights(model, model_path):
    # 미리 학습된 BART 모델 로드
    pretrained_model = torch.load(model_path)

    # BART 모델의 가중치를 현재 모델로 복사
    model.load_state_dict(pretrained_model['model_state_dict'])

def test_model(model_path, keyword_list):
    # 모델 로드
    loaded_model = BARTTextClassifier(num_labels=2)
    load_pretrained_bart_weights(loaded_model, model_path)
...
if __name__ == '__main__':
    model_path = "C:/Users/L/.spyder-py3/project/output/BART.pt"
